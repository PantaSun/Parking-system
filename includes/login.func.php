<?php
	 ...
	 */
	function _check_query_name($_string,$min_num,$max_num){
		//去掉两边空格
		$_string = trim($_string);
		if (mb_strlen($_string,'utf-8')<$min_num|| mb_strlen($_string,'utf-8')>$max_num){
			_location("用户名长度不得小于'.$min_num.'位或大于'.$max_num.'位！","query_one_ower.php");
		}
		return _mysql_string($_string);
	}
	/**
	 ...
	 */
	function _setcookies($_username,$_anth){
		setcookie('username' , $_username);
		setcookie('auth' , $_anth);
		/*switch ($_time){
			case '0':
				setcookie('username' , $_username);
